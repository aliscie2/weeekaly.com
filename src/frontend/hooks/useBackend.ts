...
export function useCalendarEvents(enabled: boolean = true) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<GoogleCalendarEvent[]> => {
      ...
      const { fetchGoogleCalendarEvents } = await import('../utils/googleCalendar');
      ...
      if (events.length > 0) {
        ...
        const currentUserEmail = firstEvent.creator?.email || firstEvent.organizer?.email;
...
export function useUpdateEvent() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { updateGoogleCalendarEvent } = await import('../utils/googleCalendar');
    ...
    onMutate: async ({ eventId, updates }) => {
      ...
      queryClient.setQueryData(['calendar-events'], (old: GoogleCalendarEvent[] | undefined) => {
        if (!old) return old;
