...
export default function App() {
  ...
  const [eventActionType, setEventActionType] = useState<
    "cancel" | "delete" | null
  >(null);
  const [pendingEventDeletion, setPendingEventDeletion] =
    useState<Event | null>(null);
  const [eventDetailsBackView] = useState<"events" | "availability">("events");
  ...
  const [events, setEvents] = useState<Event[]>([
    ...
    },
    {
      ...
      aiSummary:
        "Presenting our new software solution to TechCorp stakeholders. Demo of key features, pricing discussion, and Q&A session about implementation timeline.",
      thumbnail:
        "https://images.unsplash.com/photo-1553877522-43269d4ea984?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxvZmZpY2UlMjBjb2xsYWJvcmF0aW9ufGVufDF8fHx8MTc2MjAxOTc2N3ww&ixlib=rb-4.1.0&q=80&w=1080",
      location: "Virtual",
      createdBy: CURRENT_USER_ID, // Created by current user
    },
    {
      ...
      aiSummary:
        "Regular team sync to discuss progress updates, blockers, and priorities for the week. Quick check-in format with each team member sharing their status.",
      thumbnail:
        "https://images.unsplash.com/photo-1588196749597-9ff075ee6b5b?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx2aWRlbyUyMGNvbmZlcmVuY2V8ZW58MXx8fHwxNzYyMDU4NjQxfDA&ixlib=rb-4.1.0&q=80&w=1080",
      location: "Virtual",
...
• Amanda to prepare detailed Q4 budget breakdown by department`,
      thumbnail:
        "https://images.unsplash.com/photo-1709715357520-5e1047a2b691?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx0ZWFtJTIwbWVldGluZ3xlbnwxfHx8fDE3NjIwMjg2NDV8MA&ixlib=rb-4.1.0&q=80&w=1080",
...
• Setup training sessions for Week of Nov 8`,
  ...
  useEffect(() => {
    ...
    getDaysData(currentStartDate, count);
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#f5f3ef] via-[#ebe8e0] to-[#e8e4d9]">
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="min-h-screen flex flex-col items-center px-4 py-8 md:py-16">
              <div className="w-full max-w-2xl mx-auto space-y-8 md:space-y-12">
                ...
                >
                  No need to negotiate your time zone. Your friend lives in
                  Alaska, you live in China? Totally fine, we can figure it out.
                </motion.p>
        ...
        {/* Header - Only show for contact view */}
        {currentView === "contact" && (
          <motion.header
            initial={{ opacity: 0, y: -20 }}
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className="w-80 bg-[#f5f3ef] border-[#d4cfbe]/40">
                        {availabilities.map((availability) => {
                          ...
                          const availabilityWeekData = getDaysData(
                            startDate,
                            7,
                          );
        ...
        {currentView === "availability" && selectedAvailabilityId && (
          <AvailabilityPage
            availabilityName={
              availabilities.find((a) => a.id === selectedAvailabilityId)
                ?.name || "Availability"
            }
            currentStartDate={
              availabilities.find((a) => a.id === selectedAvailabilityId)
            ...
            onPreviousWeek={() =>
              handleAvailabilityPreviousWeek(selectedAvailabilityId)
            }
            onNextWeek={() =>
              handleAvailabilityNextWeek(selectedAvailabilityId)
            }
        ...
        {currentView === "eventDetails" && selectedEventId && (
          <EventDetailsPage
            ...
            backButtonText={
              eventDetailsBackView === "availability"
        ...
        {currentView === "deleteConfirmation" && availabilityToDelete && (
          <DeleteConfirmationPage
            itemName={
              availabilities.find((a) => a.id === availabilityToDelete)?.name ||
              "Availability"
            }
        ...
        {currentView === "eventDeleteConfirmation" && eventToDelete && (
          <DeleteConfirmationPage
            itemName={
              events.find((e) => e.id === eventToDelete)?.title || "Event"
            }
            itemType={
              eventActionType === "cancel" ? "Event Attendance" : "Event"
            }
