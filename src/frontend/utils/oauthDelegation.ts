 ...
 * @returns Promise resolving to object with identity and sessionKey on success, null on failure
 *
 * @example
 * ```typescript
...
export async function loginWithOAuth(
  ...
  try {
    ...
    // 6. Wait for callback
    const { id_token, code } = await new Promise<{
      id_token: string;
      code?: string;
    }>((resolve, reject) => {
          ...
          if (codeVerifier) {
            try {
              ...
              if ("Ok" in tokenResult) {
                ...
                const refreshToken =
                  Array.isArray(tokenResult.Ok.refresh_token) &&
                  tokenResult.Ok.refresh_token.length > 0
                ...
                const expiryTime =
                  Date.now() + Number(tokenResult.Ok.expires_in) * 1000;
                localStorage.setItem(
